...
class RelatoriosController extends Controller
{
	...
	*/
	public function todos()
	{
		...
		if ($this->acesso->getInfo('patente') <= 12) {
			$this->arrayInfo['page_active'] = 'relatorios';
			...
			$_SESSION['edit_registro'] = 'Você está tentando acessar uma página disponível apenas para os Oficiais.';
	...
	*/
	...
	public function getCriar()
	{
		...
		if (is_numeric($response['valor'])) {
			$l = new \Models\Logs();
			if ($response['valor'] == 2 && $this->acesso->getInfo('patente') > 17) {
				$tipo = 8;
				...
				$msg = "No dia " . date('d/m/Y') . " às [" . date('H:i') . "] o(a) " . $this->acesso->getInfo('nickname') . " tentou acessar um relatório que não tinha permissão.";
				$l->addLog('invasao', $msg);
			} elseif ($response['valor'] == 4 && $this->acesso->getInfo('patente') > 16) {
				$tipo = 8;
				$msg = "No dia " . date('d/m/Y') . " às [" . date('H:i') . "] o(a) " . $this->acesso->getInfo('nickname') . " tentou acessar um relatório que não tinha permissão.";
				$l->addLog('invasao', $msg);
	...
	public function addRelatorio()
	{
		...
		if (isset($_POST['tipo']) && !empty($_POST['tipo'])) {
			...
			if (
				...
				$tipo == 2 && $this->acesso->getInfo('patente') <= 19
				&& isset($_POST['data']) && !empty($_POST['data'])
				&& isset($_POST['atendente']) && !empty($_POST['atendente'])
				&& isset($_POST['responsavel']) && !empty($_POST['responsavel'])
				&& isset($_POST['novato']) && !empty($_POST['novato'])
				...
				$tipo == 3 && $this->acesso->getInfo('patente') <= 19
				&& isset($_POST['data']) && !empty($_POST['data'])
				&& isset($_POST['atendente']) && !empty($_POST['atendente'])
				...
				$tipo == 4 && $this->acesso->getInfo('patente') <= 16
				&& isset($_POST['data']) && !empty($_POST['data'])
				&& isset($_POST['atendente']) && !empty($_POST['atendente'])
	...
	public function updateStatus($status, $id)
	{
		...
		if ($this->acesso->getInfo('patente') <= 12 && in_array($status, [1, 2, 5]) && count($relatorio) > 0 && $relatorio['status'] == 0) {
