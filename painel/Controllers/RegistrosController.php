...
class RegistrosController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->acesso->getInfo('patente') >= 13) {
            $l = new \Models\Logs();
	...
	*/
    ...
    public function editRegistro($id)
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($this->acesso->podePromover($registro['patente_id'], $this->acesso->getInfo('patente')) && count($registro) > 0) {
                ...
                if ($this->acesso->podePromover($cargo, $this->acesso->getInfo('patente'))) {
                    $r->atualizarRegistro($responsvel, $nickname, $cargo, $status, $executivo, $ultima_promocao, $registro['id'], $this->acesso->getInfo('nickname'));
                    $r->addMoedas2($registro['id'], $moedas);
    ...
    public function cartao()
    {
        ...
        if ($this->acesso->getInfo('patente') <= 10) {
            $adm = ($this->acesso->getInfo('patente') <= 7)?1:2;
            $this->arrayInfo['page_active'] = 'registros';
    ...
    public function delete($id_registro)
    {
        ...
        $r = new Registros();
        $registro = $r->getMembroByID($id_registro);
        if ($this->acesso->getInfo('patente') <= 7 && is_numeric($id_registro) && count($registro) >= 0 && !in_array($id_registro, [1,2])) {
