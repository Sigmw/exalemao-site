...
class Registros extends Model
{
    ...
    public function getMembroByNickname($nickname)
    {
        ...
        $sql = $this->db->prepare("SELECT *, (select patentes.nome from patentes where patentes.id = registros.patente_id) as patente, (select patentes.executivo from patentes where patentes.id = registros.patente_id) as patente_executivo, (select status.nome from status where status.id = registros.status_id) as status FROM registros WHERE nickname = :nickname");
        $sql->bindValue(':nickname', $nickname);
        $sql->execute();
    ...
    public function getRegistros()
    {
        ...
        $sql = "SELECT *, (select patentes.nome from patentes where patentes.id = registros.patente_id) as patente, (select patentes.executivo from patentes where patentes.id = registros.patente_id) as patente_executivo FROM registros ORDER BY patente_id ASC";
    ...
    public function getRegistrosConfianca()
    {
        ...
        $sql = $this->db->prepare("SELECT *, (select patentes.nome from patentes where patentes.id = registros.patente_id) as patente, (select patentes.executivo from patentes where patentes.id = registros.patente_id) as patente_executivo, (select count(*) from confianca_voto where data_voto >= :data1 and data_voto <= :data2) as voto_semana FROM registros WHERE patente_id >= 7 ORDER BY patente_id ASC");
    ...
    public function novosAlistados() {
        $sql = $this->db->prepare("SELECT nickname FROM registros ORDER BY id DESC LIMIT 10");
        $sql->execute();
    ...
    public function getExecutivos()
    {
        ...
        $sql = "SELECT nickname FROM registros WHERE patente_id <= 12 AND status_id = 1 ORDER BY patente_id";
        $sql = $this->db->prepare($sql);
        $sql->execute();
    ...
    public function getPatentes()
    {
        ...
        $sql = "SELECT * FROM patentes ORDER BY ordem";
        $sql = $this->db->prepare($sql);
        $sql->execute();
    ...
    public function addMoedas($id_registro, $valor_moedas)
    {
        ...
        if ($valor_moedas >= 1) {
            ...
            <span>Você recebeu + U$" . $valor_moedas . "</span></a>";
    ...
    public function atualizarRegistro($responsavel, $nickname, $patente, $status, $executivo, $ultima_promocao, $id_registro, $ofc_nickname)
    {
        $sql = $this->db->prepare("UPDATE registros SET ultima_promocao = :ultima_promocao, promovido_por = :responsavel, patente_id = :patente, executivo = :executivo, status_id = :status WHERE id = :id_registro");
