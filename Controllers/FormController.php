...
class FormController extends Controller
{
	...
	*/
	...
	public function favoritarPerfil($tipo, $id_registro)
	{
		...
		if ($acesso->isLogged() && $acesso->getInfo('confirmado') != 1) {
			$l = new \Models\Logs();

			$msg = "No dia " . date('d/m/Y') . " às [" . date('H:i') . "] o(a) " . $acesso->getInfo('nickname') . " tentou acessar uma página que não tinha acesso.";
	...
	*/
	public function trocarFotoPerfil()
	{
		...
		if (
			...
			if (in_array($foto['type'], $permitidos)) {
				...
				$r->updateFoto($url_final, $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
	...
	public function alterarNome()
	{
		...
		if (
			...
			$r->updateNome($nome, $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
	...
	public function alterarEmail()
	{
		...
		if (
			...
			$r->updateEmail($email, $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
	...
	public function alterarSenha()
	{
		...
		if (
			...
			if ($senha1 == $senha2) {
				$acesso->updateSenha($senha1, $acesso->getInfo('id'), $acesso->getInfo('nickname'));
	...
	public function alterarAvatarForum()
	{
		...
		if (
			...
			if (in_array($foto['type'], $permitidos)) {
				...
				$r->updateFotoForum($url_final, $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
	...
	public function alterarDescricaoForum()
	{
		...
		if (
			...
			$r->updateDescricaoForum($texto, $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
		...
		if ($acesso->isLogged() && $acesso->getInfo('confirmado') == 1 && $acesso->getInfo('vip') != 1) {
			$l = new \Models\Logs();
	...
	public function gerarMoedas()
	{
		...
		if ($usei_codigo_moedas) {
			$l = new \Models\Logs();
			$msg = "No dia " . date('d/m/Y') . " às [" . date('H:i') . "] o(a) " . $acesso->getInfo('nickname') . " utilizou o código de moedas '".$cod_verificacao['codigo']."'.";
	...
	public function removerFotoPerfil()
	{
		...
		if (
			...
			if ($registro['avatar'] != 'https://i.imgur.com/38HBHH9.png') {
				...
				$r->updateFoto('https://i.imgur.com/38HBHH9.png', $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
			}
	...
	public function alterarAmigos()
	{
		...
		if (
			...
			$coracao = (!empty($_POST['coracao'])) ? $u->trocarItens($_POST['coracao']) : 'Yuri-Takahashi';
			$coracao_id = $r->userExiste($coracao);
			$feliz = (!empty($_POST['feliz'])) ? $u->trocarItens($_POST['feliz']) : 'Yuri-Takahashi';
			$feliz_id = $r->userExiste($feliz);
			$money = (!empty($_POST['money'])) ? $u->trocarItens($_POST['money']) : 'Yuri-Takahashi';
			...
			$m->updateAmigos($amigos, $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
	...
	public function alterarNascimento()
	{
		...
		if (
			...
			$r->updateNascimento($nascimento, $acesso->getInfo('id_registro'), $acesso->getInfo('nickname'));
