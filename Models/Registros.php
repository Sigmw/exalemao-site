...
class Registros extends Model
{
    ...
    public function getMembro($nickname)
    {
       ...
       $sql = $this->db->prepare("SELECT *, (select patentes.nome from patentes where patentes.id = registros.patente_id) as patente, (select patentes.executivo from patentes where patentes.id = registros.patente_id) as patente_executivo, (select status.nome from status where status.id = registros.status_id) as status, (select faixas.slug from faixas where faixas.id = registros.faixa) as faixa_nome FROM registros WHERE nickname LIKE :nickname");
    ...
    public function getUserByAcessoID($id) 
    {
        $array = [];
        
        $sql = $this->db->prepare("SELECT * FROM acesso WHERE id = :id");
        $sql->bindValue(':id', $id);
        $sql->execute();
