...
class Registros extends Model
{
    ...
    public function getMembro($nickname)
    {
        ...
        $sql = $this->db->prepare("SELECT *, (select patentes.nome from patentes where patentes.id = registros.patente_id) as patente, (select patentes.imagem from patentes where patentes.id = registros.patente_id) as patente_imagem, (select status.nome from status where status.id = registros.status_id) as status, (select faixas.slug from faixas where faixas.id = registros.faixa) as faixa_nome FROM registros WHERE nickname LIKE :nickname");
    ...
    public function getMembroByNickname($nickname)
    {
        ...
        $sql = $this->db->prepare("SELECT *, (select patentes.nome from patentes where patentes.id = registros.patente_id) as patente, (select patentes.imagem from patentes where patentes.id = registros.patente_id) as patente_imagem, (select status.nome from status where status.id = registros.status_id) as status FROM registros WHERE nickname = :nickname");
        $sql->bindValue(':nickname', $nickname);
        $sql->execute();
    ...
    public function getHonrariasUser($nickname) {
        
        $array = [];

        $sql = $this->db->prepare("SELECT honraria_id FROM honrarias_users WHERE nickname = :nickname");
        $sql->bindValue(':nickname', $nickname);
        $sql->execute();
    ...
    public function getHonrariaInfo($id)
    {
        $array = [];

        $sql = $this->db->prepare("SELECT * FROM honrarias WHERE id = :id");
        $sql->bindValue(':id', $id);
        $sql->execute();
    ...
    public function getAllMembros()
    {
            ...
            # pegar todas patentes
      $getPatentes = $this->db->prepare("SELECT * FROM patentes ORDER BY id ASC");
      $getPatentes->execute();
      $resPatentes = $getPatentes->fetchAll();
        foreach($resPatentes as $rPatentes) {
        $patenteId = $rPatentes['id'];
        
        
                    # pegar todos os usuarios q tem aquela patente
        
                    $getAlistados = $this->db->prepare("SELECT * FROM membros WHERE patente_id = :patente_id");
                    $getAlistados->bindValue(':patente_id', $patenteId);
                    $getAlistados->execute();
                    $resAlistados = $getAlistados->fetchAll();
                    
                    foreach($resAlistados as $rAlistados) {
                        return $rAlistados['nickname'];
                    }
